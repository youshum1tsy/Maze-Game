...
void Menu::Load() {
	if (backgroundTexture.loadFromFile("../Assets/Menu/main_menu.png")) {
		backgroundSprite.setTexture(backgroundTexture);
		backgroundSprite.setPosition(0, 0);
	}
	else {
		std::cout << "error when loading background menu" << std::endl;
	}
...
void Menu::Update(sf::RenderWindow& window) {
	...
	if (sf::Mouse::isButtonPressed(sf::Mouse::Left)) {
		if (menuNum == MenuConstants::PLAY) {
			isMenu = false;
		}
		if (menuNum == MenuConstants::QUIT) {
			window.close();
			isMenu = false;
		}
	}
